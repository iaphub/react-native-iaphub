...
class Iaphub {
	...
	async init(opts = {}) {
		...
		if (!opts.onPurchaseSuccess) {
			throw this.error("Missing onPurchaseSuccess option", 'on_purchase_success_empty');
		}
		if (!opts.onPurchaseError) {
			throw this.error("Missing onPurchaseError option", 'on_purchase_error_empty');
		}
	...
	async buy(productSku) {
		...
		try {
			...
			if (!product) {
				throw this.error(`Buy failed, product sku not in the products for sale`, 'sku_not_found');
			}
			...
			throw this.error(`Buy failed (Err: ${err.message})`, err.code || 'unknown');
		}
	}
	...
	async getUser(params = {}) {
		...
		var productIds = products.filter((item) => item.type.indexOf('renewable_subscription') == -1).map((item) => item.sku);
		var subscriptionIds = products.filter((item) => item.type.indexOf('renewable_subscription') != -1).map((item) => item.sku);
		var productsInfos = [];
		...
		if (subscriptionIds.length) {
			productsInfos = productsInfos.concat(await RNIap.getSubscriptions(subscriptionIds));
		}
		...
		try {
			await this.setPricing([].concat(this.user.productsForSale).concat(this.user.activeProducts));
		} catch (err) {
			console.error(err);
		}
		return this.user;
	...
	async setUserTags(tags) {
		...
		try {
			...
			throw this.error(`Set user tags failed (Err: ${err.message})`, err.code || 'unknown');
		}
	}
	...
	async restore() {
		...
		try {
			...
			availablePurchases.forEach((purchase) => {
				var hasDuplicate = purchases.find((item) => this.getReceiptToken(item) == this.getReceiptToken(purchase));
			...
			throw this.error(`Restore failed (Err: ${err.message})`, err.code || 'unknown');
		}
	}
	...
	async processReceipt(purchase, isRestore = false) {
		var receipt = {token: this.getReceiptToken(purchase), sku: purchase.productId, isRestore: isRestore};
		var newTransactions = [];
		var oldTransactions = [];
		...
		try {
			var response = await this.request('post', '/receipt', {token: receipt.token, sku: receipt.sku});
			...
			// Otherwise emit receipt error event (A restore won't be needed, IAPHUB will retry processing the receipt)
				...
				shouldFinishReceipt = true;
				newTransactions = await this.emitReceiptError(this.error('Receipt validation failed', 'receipt_validation_error'), receipt);
		...
		if (shouldFinishReceipt) {
			var productType = await this.detectProductType(receipt.sku, newTransactions, oldTransactions);
	...
	async request(type, url = '', params = {}) {
		...
		try {
			var response = await fetch(`${this.apiUrl}/app/${this.appId}/user/${this.userId}${url}`, opts);
			json = await response.json();
		} catch (err) {
			throw this.error(`Network error, request to the Iaphub API failed (${err.message})`, 'network_error');
		}
